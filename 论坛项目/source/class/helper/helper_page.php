...
class helper_page {
	...
	public static function multi($num, $perpage, $curpage, $mpurl, $maxpages = 0, $page = 10, $autogoto = FALSE, $simple = FALSE, $jsfunc = FALSE) {
		...
		if($num > $perpage) {
			...
			*/
			$multipage = ($curpage - $offset > 1 && $pages > $page ? '<a href="'.(self::mpurl($mpurl, $pagevar, 1)).$a_name.'" class="first"'.$ajaxtarget.'>1 '.$dot.'</a>' : '').
			($curpage > 1 && !$simple ? '<td><a href="'.(self::mpurl($mpurl, $pagevar, $curpage - 1)).$a_name.'" class="prev"'.$ajaxtarget.'><div class="circle orange  "><i class="fa fa-angle-double-left" aria-hidden="true"></i></div></a></td>' : '');
			for($i = $from; $i <= $to; $i++) {
				$multipage .= $i == $curpage ? '<td><a href="'.(self::mpurl($mpurl, $pagevar, $i)).($ajaxtarget && $i == $pages && $autogoto ? '#' : $a_name).'"'.$ajaxtarget.'><div class="circle orange-bg pagination-active" > '.$i.'</div></a></td>':
				'<td><a href="'.(self::mpurl($mpurl, $pagevar, $i)).($ajaxtarget && $i == $pages && $autogoto ? '#' : $a_name).'"'.$ajaxtarget.'><div class="circle orange" >'.$i.'</div></a></td>';
			}
			...
			$multipage .= ($to < $pages ? '<td><a href="'.(self::mpurl($mpurl, $pagevar, $pages)).$a_name.'" class="last"'.$ajaxtarget.'>'.$dot.' '.$realpages.'</a></td>' : '').
            //($showpagejump && !$simple && !$ajaxtarget && !$wml ? '<label><input type="text" name="custompage"   class="px" size="2" title="'.$lang['pagejumptip'].'" value="'.$curpage.'" onkeydown="if(event.keyCode==13) {window.location=\''.$jsurl.'}" /><span title="'.$lang['total'].' '.$pages.' '.$lang['pageunit'].'"> / '.$pages.' '.$lang['pageunit'].'</span></label>' : '').
			($curpage < $pages && !$simple ? '<td><a href="'.(self::mpurl($mpurl, $pagevar, $curpage + 1)).$a_name.'" class="nxt"'.$ajaxtarget.'><div class="circle orange" ><i class="fa fa-angle-double-right" aria-hidden="true"></i></div></a></td>' : '').
			($showkbd && !$simple && $pages > $page && !$ajaxtarget && !$wml ? '<kbd><input type="text" name="custompage" size="3" onkeydown="if(event.keyCode==13) {window.location=\''.$jsurl.'}" /></kbd>' : '');

			$multipage = $multipage ? '<table class="pagination"><tr>'.($shownum && !$simple ? '<em>&nbsp;'.$num.'&nbsp;</em>' : '').$multipage.'</tr></table>' : '';
		}
		$maxpage = $realpages;
		return $multipage;
	}
	...
	public static function simplepage($num, $perpage, $curpage, $mpurl) {
		...
		$lang['next'] = lang('core', 'nextpage');
		$lang['prev'] = lang('core', 'prevpage');
		$next = $num == $perpage ? '<td><a href="'.(self::mpurl($mpurl, '&amp;page=', $curpage + 1)).'" class="nxt"><div class="circle orange" ><i class="fa fa-angle-double-right" aria-hidden="true"></i></div> </a></td>' : '';
		$prev = $curpage > 1 ? '<td><span class="pgb"><a href="'.(self::mpurl($mpurl, '&amp;page=', $curpage - 1)).'"><div class="circle orange  " ><i class="fa fa-angle-double-left" aria-hidden="true"></i></div></a></span></td>' : '';
		if($next || $prev) {
