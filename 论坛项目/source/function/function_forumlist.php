...
function forum(&$forum) {
	...
	if($forum['icon']) {
		...
		$forum['icon'] = '<a href="forum.php?mod=forumdisplay&fid='.$forum['fid'].'" zhangjiwei1><img src="'.$forum['icon'].'" align="left" alt="" /></a>';
	}
...
function moddisplay($moderators, $type, $inherit = 0) {
	if($moderators) {
		...
		foreach(explode("\t", $moderators) as $moderator) {
			$query = DB::fetch_first("SELECT * FROM ".DB::table('common_member')." WHERE username='".$moderator."'"); 
			$uid = $query['uid'];  
			$image =avatar($uid,'small');
			$image = str_replace('<img', '<img class="moderator-image"', $image);
			$modlist .= '<div class="moderator">'.$image.'<span><a href="home.php?mod=space&username='.rawurlencode($moderator).'" class="notabs " c="1">'
			.($inherit ? '<strong>'.$moderator.'</strong>' : $moderator).'</a></span></div>';
